...
class TestClientConstruction:
    ...
    def test_account_level_resources(self):
        transport = httpx.MockTransport(lambda r: httpx.Response(200))
        client = httpx.Client(transport=transport)
        ln = LnBot(api_key="k", http_client=client)
    ...
    def test_wallet_returns_scope(self):
        transport = httpx.MockTransport(lambda r: httpx.Response(200))
        client = httpx.Client(transport=transport)
        ln = LnBot(api_key="k", http_client=client)
        w = ln.wallet("wal_1")
        ...
        assert w.invoices is not None
        assert w.payments is not None
        assert w.addresses is not None
        assert w.transactions is not None
        assert w.webhooks is not None
        assert w.events is not None
        assert w.l402 is not None

    def test_wallet_rejects_empty(self):
        transport = httpx.MockTransport(lambda r: httpx.Response(200))
        client = httpx.Client(transport=transport)
        ln = LnBot(api_key="k", http_client=client)
...
class TestHeaders:
    def test_sends_authorization(self):
        ln, cap = create_client(json_body={"userId": "usr_1", "primaryKey": "uk_1", "secondaryKey": "uk_2", "recoveryPassphrase": "a b"})
        ln.register()
    ...
    def test_omits_auth_when_no_key(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            body = json.dumps({"userId": "usr_1", "primaryKey": "uk_1", "secondaryKey": "uk_2", "recoveryPassphrase": "a b"})
    ...
    def test_sends_content_type_for_post(self):
        ...
        ln.wallet("wal_1").invoices.create(amount=100)
...
class TestHTTPMethods:
    ...
    def test_patch(self):
        ...
        ln.wallet("wal_1").update(name="Renamed")
    ...
    def test_register(self):
        ...
        result = ln.register()
        assert cap.method == "POST"
        assert cap.path == "/v1/register"
        assert result.user_id == "usr_1"

    def test_me(self):
        ln, cap = create_client(json_body={"walletId": "wal_1"})
        result = ln.me()
        assert cap.method == "GET"
        assert cap.path == "/v1/me"
        assert result.wallet_id == "wal_1"


class TestRequestBody:
    def test_serializes_json(self):
        ln, cap = create_client(json_body={"number": 1, "status": "pending", "amount": 100, "bolt11": "lnbc1..."})
        ln.wallet("wal_1").invoices.create(amount=100, memo="test memo")
        body = cap.json_body
    ...
    def test_converts_to_camel_case(self):
        ...
        ln.wallet("wal_1").payments.create(target="user@ln.bot", idempotency_key="idem_1")
        body = cap.json_body
